...
(mcffi:with-doc-file (doc-file (asdf:system-relative-pathname "common-vulkan" "docs/api/structs.md"))
  ...
  (more-cffi:def-foreign-struct doc-file "VkPhysicalDeviceMemoryProperties" (physical-device-memory-properties)
    ...
    (memorytypes :pointer t :name "memoryTypes" :type (list "VkMemoryType")
		 :create ((memorytypes-arg)
			  (create-array (:struct vkmemorytype) memorytypes memorytypes-arg :dynamic nil :pointers t))
		 :get ((&optional memorytypes-index)
		       (get-array (:struct vkmemorytype) memorytypes memorytypes-index 32 :pointers t))
		 :set ((memorytypes-arg &optional memorytypes-index)
		       (set-array (:struct vkmemorytype) memorytypes memorytypes-arg memorytypes-index :dynamic nil :pointers t)))
    (memoryheapcount :name "memoryHeapCount" :type uint32)
    (memoryheaps :pointer t :name "memoryHeaps" :type (list "VkMemoryHeap")
		 :create ((memoryheaps-arg)
			  (create-array (:struct vkmemoryheap) memoryheaps memoryheaps-arg :dynamic nil :pointers t))
		 :get ((&optional memoryheaps-index)
		       (get-array (:struct vkmemoryheap) memoryheaps memoryheaps-index 16 :pointers t))
		 :set ((memoryheaps-arg &optional memoryheaps-index)
		       (set-array (:struct vkmemoryheap) memoryheaps memoryheaps-arg memoryheaps-index :dynamic nil :pointers t))))
  ...
  (more-cffi:def-foreign-struct doc-file "VkDescriptorSetLayoutBinding" (descriptor-set-layout-binding)
    ...
    (stageflags :name "stageFlags" :type "VkShaderStageFlags")
    (pimmutablesamplers :name "pImmutableSamplers" :type (list "VkSampler") :init-form nil
			:create ((pimmutablesamplers-arg)
				 (create-array :uint32 pimmutablesamplers pimmutablesamplers-arg :dynamic t :pointers nil))
			:destroy (destroy-array pimmutablesamplers)
			:get ((&optional pimmutablesamplers-index)
			      (get-array :uint32 pimmutablesamplers pimmutablesamplers-index queuefamilyindexcount :pointers nil))
			:set ((pimmutablesamplers-arg &optional pimmutablesamplers-index)
			      (set-array :uint32 pimmutablesamplers pimmutablesamplers-arg pimmutablesamplers-index :dynamic t :pointers nil))))
  ...
  (more-cffi:def-foreign-struct doc-file "VkDescriptorSetLayoutCreateInfo" (descriptor-set-layout-create-info)
    ...
    (pbindings :name "pBindings" :type (list "VkDescriptorSetLayoutBinding") :init-form nil
	       :create ((pbindings-arg)
			(create-array (:struct vkdescriptorsetlayoutbinding) pbindings pbindings-arg :dynamic t :pointers t))
	       :destroy (destroy-array pbindings)
	       :get ((&optional pbindings-index)
		     (get-array (:struct vkdescriptorsetlayoutbinding) pbindings pbindings-index bindingcount :pointers t))
	       :set ((pbindings-arg &optional pbindings-index)
		     (set-array (:struct vkdescriptorsetlayoutbinding) pbindings pbindings-arg pbindings-index :dynamic t :pointers t)))))
