...
(defun get-queue-families (physical-device-ptr flags min-queue-count surface-ptr exclusive-flags family-exceptions)
  (with-queue-family-properties ((queue-family-properties-ptr family-queue-count) vk-physical-device)
    (loop for i from 0 below family-queue-count
          ...
          if (and (implies (> queue-flags 0) (equal (logand property-flags queue-flags) queue-flags))
                  ...
                  (implies surface-ptr (cffi:with-foreign-object (present-queue-ptr 'VkBool32)
                                        (vkGetPhysicalDeviceSurfaceSupportKHR vk-physical-device i surface-ptr present-queue-ptr)
            ...
            collect (make-queue-family :index i :queue-flags property-flags :queue-count property-queue-count))))
  ...
  (with-queue-family-properties ((queue-family-properties-ptr family-queue-count) vk-physical-device)
    (loop for i from 0 below family-queue-count
          ...
          if (and (implies (> queue-flags 0) (equal (logand property-flags queue-flags) queue-flags))
                  ...
                  (not (loop for family-exception in family-exceptions
                             thereis (equal (queue-family-index family-exception) i))))
            return (make-queue-family :index i :queue-flags property-flags :queue-count property-queue-count))))
...
;; Returns a list of family queues that verifies some conditions
(defun vk-physical-device-queue-families (physical-device &key (queue-flags 0) (min-queue-count 0) (surface nil)
...
;; Returns a family queue that verifies some conditions
(defun vk-physical-device-queue-family (physical-device &key (queue-flags 0) (min-queue-count 0) (surface nil)
                                                             (exclusive-flags nil) (family-exceptions nil))
  (let ((physical-device-ptr (vk-physical-device-physical-device-ptr physical-device))
        (surface-ptr (vk-surface-surface-ptr surface)))
