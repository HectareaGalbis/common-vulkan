...
(defun main ()
  ...
  (let* ((instance        (cvk:create-instance t))
    ...
    (multiple-value-bind  (swapchain img-format) (cvk:create-swapchain physical-device device surface (list queue-family) 640 480)
      (let* ((module (cvk:create-shader-module device "shaders/shader/vert.spv"))
             ...
             (subpass (cvk:create-subpass-description cvk:VK_PIPELINE_BIND_POINT_GRAPHICS :color-attachments (list color-reference)))
             (render-pass (cvk:create-render-pass device (list color-attachment) (list color-reference))))
        (format t "Hola~%")
        (sleep 3)
        (format t "Adios~%")
