...
(let ((callbacks-count (defined-callbacks-count)))
  (multiple-value-bind (cfunctions-count functions-count) (defined-functions-count)
	...
	(adp:itemize (adp:item "Callbacks: " callbacks-count "/8" " (" (floor (* 100 callbacks-count) 8) "%)")
		     (adp:item "Functions: " functions-count "/" cfunctions-count " (" (floor (* 100 functions-count) cfunctions-count) "%)")
		     (adp:item "Structs: " structs-count "/" cstructs-count " (" (floor (* 100 structs-count) cstructs-count) "%)")
		     (adp:item "Total: " total-count "/" ctotal-count " (" (floor (* 100 total-count) ctotal-count) "%)"))))))
